...
class GoogleCalendarManager:
    ...
    def _authenticate(self):
        creds = None
        # Ưu tiên đọc từ biến môi trường (cho GitHub Actions)
        env_token = os.getenv("GOOGLE_TOKEN_JSON")
        if env_token:
            creds = Credentials.from_authorized_user_info(json.loads(env_token), self.scopes)
        
        # Nếu không có (chạy ở local), thì đọc file token.json như cũ
